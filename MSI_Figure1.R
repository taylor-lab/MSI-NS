...
# load sample data
msi.master <-
  as.data.table(suppressWarnings(
    readxl::read_excel("data/extended_data_tables.xlsx", sheet = 1)
  ))
...
for(col in num_cols) {set(msi.master, j = col, value = as.numeric(msi.master[[col]]))}

# Fig 1a

# MSI score

tmp.dat = msi.master[!is.na(MSI), .(
...
tmp.dat = msi.master[MSI != "UNK", .(
  min = log10(min(TMB + 0.01, na.rm = T)),
  q1 = log10(quantile(TMB + 0.01, probs = 0.25, na.rm = T)),
  med = log10(quantile(TMB + 0.01, probs = 0.5, na.rm = T)),
  q3 = log10(quantile(TMB + 0.01, probs = 0.75, na.rm = T)),
  max = log10(max(TMB + 0.01, na.rm = T))
), by = MSI]
...
tmp.dat = msi.master[MSI != "UNK" & (n_mut > 9), .(
  min = min(indel_prop, na.rm = T),
  q1 = quantile(indel_prop, probs = 0.25, na.rm = T),
  med = quantile(indel_prop, probs = 0.5, na.rm = T),
  q3 = quantile(indel_prop, probs = 0.75, na.rm = T),
  max = max(indel_prop, na.rm = T)
), by = MSI]
...
ggplot(msi_quantile_dat[N > 10 & MSI == "MSI"], aes(x = msi,
  ...
  geom_point(size = 5,
             ...
             aes(fill = factor(MSI_class,
                               levels = c("TRUE", "FALSE")))) +
  geom_segment(
    data = msi_quantile_dat[N > 10 & MSI == "MSI"],
    color = "black",
    aes(
      y = 100 * hap,
      yend = 100 * hap,
  ...
  scale_fill_manual(
    "",
    values = c("TRUE" = brewer.pal(n=9, "Purples")[7],
               "FALSE" = 'lightgrey'),
...
ggplot(msi.master[MSI != "UNK", .N,
       ...
       aes(
         ...
         y = 100 * pct,
         fill = factor(WGD, levels = rev(c("Yes", "No")))
       )) +
  geom_bar(stat = "identity", width = 0.8) +
  scale_fill_manual("WGD",
                    values = c("No" = "lightgrey",
                               "Yes" = "black")) +
  scale_y_continuous(
    limits = c(0, 50),
    expand = c(0, 0),
